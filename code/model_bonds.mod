// Problem Set 1 - Fiscal policy in the RBC model (with bonds)

// AUXILIAR SCRIPT
// This script is NOT meant to be run. Instead, RUN "assigment1main.m" SCRIPT

var c l k b y w rk rb T z g;
varexo eps_z eps_g;
...
model;
  ...
  sigma*c(+1) = sigma*c + beta*rbbar*rb(+1);
  rkbar*rk = rbbar*rb;
  ...
  bbar*b = (1+rbbar)*bbar*b(-1) + rbbar*bbar*rb + gbar*g - Tbar*T;
  T = phiTb*b(-1);
  z = rhoz*z(-1) + eps_z;
  g = rhog*g(-1) + eps_g;
...
stoch_simul(order=1, irf=150) y l c k rb b T;
